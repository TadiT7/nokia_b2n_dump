...
fWLANmode() {
    ...
    echo "format = perist.sys.wbg.wifi.format = " $format       >> $LOGFILENAME
    echo "BAND_Width = persist.sys.wbg.wifi.ht = " $BAND_Width  >> $LOGFILENAME
...
fRateindex() {
    Rateindex=0
    echo RATE = persist.sys.wbg.wifi.rate =  $RATE >> $LOGFILENAME
...
fWlanATSetWifiFreq() {
    channel_no=$(getprop persist.sys.wbg.wifi.channel)
    freq=0
...
fWlanATSetWifiAntenna() {
    ...
    Antenna=$(getprop persist.sys.wbg.wifi.Antenna)
...
fWlanATSetWifiBand() {
    ...
    BAND_Width=`getprop persist.sys.wbg.wifi.ht`
...
WiFi_txmode() {
    if [ $1 == "ON" ]; then
        ...
        TxPower=$(getprop persist.sys.wbg.wifi.power)
...
WiFi_cwmode() {
    if [ $1 == "ON" ]; then
        ...
        Gain_index=$(getprop persist.sys.wbg.wifi.cw_rg)
        Dac_gain=$(getprop persist.sys.wbg.wifi.cw_dg)
...
echo "Mode (persist.sys.wbg.wifi.mode):"$MODE                   >> $LOGFILENAME
# intend to show this message to terminal screen
echo "Mode:"$MODE

# (02) STANDARD
STANDARD=`getprop persist.sys.wbg.wifi.standard`
echo "Standard (persist.sys.wbg.wifi.standard):"$STANDARD       >> $LOGFILENAME
echo "Standard:"$STANDARD

# (03) format
format=`getprop persist.sys.wbg.wifi.format`
echo "format (persist.sys.wbg.wifi.format):"$format             >> $LOGFILENAME

# (04) CHANNEL
CHANNEL=`getprop persist.sys.wbg.wifi.channel`
echo "CHANNEL (persist.sys.wbg.wifi.channel):"$CHANNEL          >> $LOGFILENAME

# (05) POWER
POWER=`getprop persist.sys.wbg.wifi.power`
echo "POWER (persist.sys.wbg.wifi.power):"$POWER                >> $LOGFILENAME
...
if [ "$STANDARD" == "802.11n" ]; then
    ...
    echo "HT (persist.sys.wbg.wifi.ht):"$HT                     >> $LOGFILENAME
fi

# (09) WLFILENAME
WLFILENAME=`getprop persist.sys.wbg.wifi.wlfile`
echo "WLFILENAME (persist.sys.wbg.wifi.wlfile):"$WLFILENAME     >> $LOGFILENAME

# (10) FUNCTION
FUNCTION=`getprop persist.sys.wbg.wifi.function`
echo "FUNCTION (persist.sys.wbg.wifi.function):"$FUNCTION       >> $LOGFILENAME

# (11) Antenna
Antenna=`getprop persist.sys.wbg.wifi.Antenna`
echo "Antenna (persist.sys.wbg.wifi.Antenna):"$Antenna       >> $LOGFILENAME

# (12) Gain_index
Gain_index=$(getprop persist.sys.wbg.wifi.cw_rg)
echo "Gain_index (persist.sys.wbg.wifi.cw_rg):"$Gain_index       >> $LOGFILENAME

# (13) Dac_gain
Dac_gain=$(getprop persist.sys.wbg.wifi.cw_dg)
echo "Dac_gain (persist.sys.wbg.wifi.cw_dg):"$Dac_gain       >> $LOGFILENAME
